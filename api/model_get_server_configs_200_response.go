/*
ODIN Fleet API

API to access Odin Fleet, empowering developers to deploy gameservers all around the world in just a few lines of code.  To authenticate, users must send their email, password, and session ID to the **[`/api/auth`](/operations/getAuthToken)** endpoint. Upon successful authentication, an API token will be provided. This token should be included in the `Authorization` header as a Bearer token for all subsequent requests.      Alternatively, you can copy your API key directly from our **[`console`](https://console.4players.io)** by opening any app, and then going to **Settings -> API Keys**.

API version: 0.9.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the GetServerConfigs200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetServerConfigs200Response{}

// GetServerConfigs200Response struct for GetServerConfigs200Response
type GetServerConfigs200Response struct {
	Data  []ServerConfig                         `json:"data"`
	Links GetAppLocationSettings200ResponseLinks `json:"links"`
	Meta  GetAppLocationSettings200ResponseMeta  `json:"meta"`
}

type _GetServerConfigs200Response GetServerConfigs200Response

// NewGetServerConfigs200Response instantiates a new GetServerConfigs200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetServerConfigs200Response(data []ServerConfig, links GetAppLocationSettings200ResponseLinks, meta GetAppLocationSettings200ResponseMeta) *GetServerConfigs200Response {
	this := GetServerConfigs200Response{}
	this.Data = data
	this.Links = links
	this.Meta = meta
	return &this
}

// NewGetServerConfigs200ResponseWithDefaults instantiates a new GetServerConfigs200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetServerConfigs200ResponseWithDefaults() *GetServerConfigs200Response {
	this := GetServerConfigs200Response{}
	return &this
}

// GetData returns the Data field value
func (o *GetServerConfigs200Response) GetData() []ServerConfig {
	if o == nil {
		var ret []ServerConfig
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *GetServerConfigs200Response) GetDataOk() ([]ServerConfig, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *GetServerConfigs200Response) SetData(v []ServerConfig) {
	o.Data = v
}

// GetLinks returns the Links field value
func (o *GetServerConfigs200Response) GetLinks() GetAppLocationSettings200ResponseLinks {
	if o == nil {
		var ret GetAppLocationSettings200ResponseLinks
		return ret
	}

	return o.Links
}

// GetLinksOk returns a tuple with the Links field value
// and a boolean to check if the value has been set.
func (o *GetServerConfigs200Response) GetLinksOk() (*GetAppLocationSettings200ResponseLinks, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Links, true
}

// SetLinks sets field value
func (o *GetServerConfigs200Response) SetLinks(v GetAppLocationSettings200ResponseLinks) {
	o.Links = v
}

// GetMeta returns the Meta field value
func (o *GetServerConfigs200Response) GetMeta() GetAppLocationSettings200ResponseMeta {
	if o == nil {
		var ret GetAppLocationSettings200ResponseMeta
		return ret
	}

	return o.Meta
}

// GetMetaOk returns a tuple with the Meta field value
// and a boolean to check if the value has been set.
func (o *GetServerConfigs200Response) GetMetaOk() (*GetAppLocationSettings200ResponseMeta, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Meta, true
}

// SetMeta sets field value
func (o *GetServerConfigs200Response) SetMeta(v GetAppLocationSettings200ResponseMeta) {
	o.Meta = v
}

func (o GetServerConfigs200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetServerConfigs200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	toSerialize["links"] = o.Links
	toSerialize["meta"] = o.Meta
	return toSerialize, nil
}

func (o *GetServerConfigs200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
		"links",
		"meta",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetServerConfigs200Response := _GetServerConfigs200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varGetServerConfigs200Response)

	if err != nil {
		return err
	}

	*o = GetServerConfigs200Response(varGetServerConfigs200Response)

	return err
}

type NullableGetServerConfigs200Response struct {
	value *GetServerConfigs200Response
	isSet bool
}

func (v NullableGetServerConfigs200Response) Get() *GetServerConfigs200Response {
	return v.value
}

func (v *NullableGetServerConfigs200Response) Set(val *GetServerConfigs200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetServerConfigs200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetServerConfigs200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetServerConfigs200Response(val *GetServerConfigs200Response) *NullableGetServerConfigs200Response {
	return &NullableGetServerConfigs200Response{value: val, isSet: true}
}

func (v NullableGetServerConfigs200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetServerConfigs200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}